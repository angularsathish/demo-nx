...
@Component({
  selector: 'demo-nx-lookup-student',
  templateUrl: './lookup-student.component.html',
  styleUrls: ['./lookup-student.component.scss'],
})
export class LookupStudentComponent implements OnInit {
  columns = [
  ...
  onTableAction(event: TableButtonAction) {
    ...
    switch (event.name) {
      case 'ADD':
        this.router.navigate(['/' + rolename + '/people/student/add']);
        break;
      ...
      case 'VIEW':
        this.router.navigate(['/' + rolename + '/people/student/profile']);
        break;
