...
@Component({
  selector: 'demo-nx-lookup-department',
  templateUrl: './lookup-department.component.html',
  styleUrls: ['./lookup-department.component.scss'],
})
export class LookupDepartmentComponent implements OnInit {
  columns = [
  ...
  onTableAction(event: TableButtonAction) {
    ...
    switch (event.name) {
      case 'ADD':
        this.router.navigate(['/' + rolename + '/people/parent/add']);
        break;
      ...
      case 'VIEW':
        this.router.navigate(['/' + rolename + '/people/parent/profile']);
        break;
